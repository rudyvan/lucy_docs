...
/* iButton reader */
// loxone reader wire white AND do not forget to connect this pin with 4,7K ohm resistor to 5V
// int grnd_pin     = // loxone reader wire yellow and grey
// below 2 one wire inputs are allocated
int w1_pin1 = 6;
int w1_pin2 = 7;
OneWire w1_array[] = {(w1_pin1), (w1_pin2)};
String was_button="";
String cur_button="";
...
void setup() {
  ...
  for (byte i = 0; i < A_nr_in_pins; i = i + 1) {
    pinMode(A_in_pins[i], INPUT); // sets the analog pin as input
    A_in_state[i] = 0; // analogRead(A_in_pins[i]); // reads the initial state 
  }
}
...
int lp_button(int w1_nr) {
  ...
  if (!w1_array[w1_nr].search(addr)) {
    w1_array[w1_nr].reset_search();
    return 0;
  }  
  ...
  if (cur_button!=was_button) {
    ...
    print_button(w1_nr); // it prints the button  
...
int latch_A_in(){ // read analog input and send serial message if it changes
  for (byte i = 0; i < A_nr_in_pins; i = i + 1) {
    int new_state = analogRead(A_in_pins[i]); // read the initial state
    if ( abs(new_state-A_in_state[i]) > 50 ) {   // new state when 0,5% deviation
      A_i_str[1]='0'+i;
      int new_percent = new_state*100L/(1023-110);
      A_i_str[3]='0'+new_percent / 100;
      A_i_str[4]='0'+(new_percent / 10) % 10;
      A_i_str[5]='0'+new_percent % 10;
      Serial.println(A_i_str);
      A_in_state[i]=new_state;
    }
  }
}
...
int latch_serial(){
  if (stringComplete){
    if (inputString[0]=='a') {
      ...
      if (pin < A_nr_in_pins) {
        A_i_str[1]=inputString[1];
        i_str[3]='0'+A_in_state[pin];
        int new_state = analogRead(A_in_pins[pin]);
        int new_percent = new_state*100L/(1023-110);
        A_i_str[3]='0'+new_percent / 100;
        A_i_str[4]='0'+(new_percent / 10) % 10;
        A_i_str[5]='0'+new_percent % 10;
...
void loop(void) {
  lp_button(0);   // 1wire button reader on 1st channel
  lp_button(1);   // 1wire button reader on 2nd channel
  latch_in();     // sample inputs 
  latch_A_in();   // sample analog inputs 
